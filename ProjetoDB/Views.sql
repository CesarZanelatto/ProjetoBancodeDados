...
SELECT
	...
	SUM(nb.STATUS = 'indisponivel') AS INDISPONIVEIS
FROM setor s
...
-- View 4: Funcionários com seus notebooks
CREATE OR REPLACE VIEW vw_funcionarios_notebooks AS
SELECT
	f.ID_FUNCIONARIO,
	f.NOME,
	f.CARGO,
	s.nome_setor,
COUNT(nb.ID_NOTEBOOK) AS QTD_NOTEBOOKS
FROM FUNCIONARIO f
JOIN setor s ON f.ID_SETOR = s.id_setor
LEFT JOIN NOTEBOOK nb ON f.ID_FUNCIONARIO = nb.ID_FUNCIONARIO
GROUP BY f.ID_FUNCIONARIO, f.NOME, f.CARGO, s.nome_setor;